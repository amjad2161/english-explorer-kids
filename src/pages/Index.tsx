...
const itemVariants = {
  ...
  visible: { y: 0, opacity: 1, scale: 1, transition: { type: "spring" as const, stiffness: 120, damping: 18 } },
};
...
const Index = () => {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden" dir={dir}>
      ...
      {/* Flowing organic aurora blobs */}
      <div className="fixed inset-0 pointer-events-none z-0">
        <motion.div
          className="absolute rounded-full"
          style={{
            width: 900, height: 900,
            background: "radial-gradient(ellipse, hsl(var(--primary) / 0.06), hsl(var(--candy) / 0.03), transparent 65%)",
            top: "-25%", right: "-20%",
        ...
        <motion.div
          ...
          style={{
            ...
            background: "radial-gradient(ellipse, hsl(var(--lavender) / 0.05), hsl(var(--accent) / 0.03), transparent 65%)",
            bottom: "-15%", left: "-15%",
      ...
      >
        ...
        <motion.div variants={itemVariants} className="text-center mb-10 relative">
            ...
            <AnimatePresence>
              {mascotMood === "wave" && (
                <motion.div
                  ...
                  className="absolute top-0 end-0 bg-card/90 backdrop-blur-2xl rounded-3xl px-5 py-2.5 shadow-xl border border-border/30 z-20"
          ...
          <motion.h1
            className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-display font-extrabold mb-4 leading-tight tracking-tight"
            style={{
          ...
          <motion.p
            className="text-lg sm:text-xl md:text-2xl text-muted-foreground font-body max-w-xl mx-auto leading-relaxed"
        ...
        <motion.div variants={floatVariants} className="flex justify-center gap-3 sm:gap-4 mb-8 flex-wrap">
          {[
            { emoji: "⭐", value: totalStars, label: t("home.stars"), bg: "from-sunshine/15 to-primary/5" },
            { emoji: "🏅", value: badgeCount, label: lang === "he" ? "הישגים" : lang === "ar" ? "إنجازات" : "Badges", bg: "from-candy/15 to-lavender/5" },
            { emoji: level.emoji, value: `${t("home.level")} ${level.id}`, label: t(`level.${level.id}`), bg: "from-primary/15 to-candy/5" },
          ].map((stat, i) => (
            <motion.div
              key={i}
              whileHover={{ scale: 1.1, y: -6, rotate: i === 1 ? 2 : -2 }}
              className={`card-glass flex items-center gap-3 px-5 sm:px-6 py-3.5 rounded-3xl bg-gradient-to-br ${stat.bg} cursor-default`}
        ...
        <motion.div variants={floatVariants} className="max-w-xl mx-auto mb-6">
          <motion.div 
            className="flex items-center gap-3 justify-center py-3 px-6 rounded-full bg-gradient-to-r from-primary/5 via-candy/5 to-lavender/5"
          ...
          >
            ...
            <span className="font-display font-bold text-sm sm:text-base">{motivation.text}</span>
          </motion.div>
        </motion.div>
        ...
        {recommendations.length > 0 && (
          <motion.div variants={floatVariants} className="max-w-xl mx-auto mb-8">
            <h3 className="font-display text-base font-bold mb-3 text-center text-muted-foreground section-title mx-auto">
            ...
            <div className="space-y-2.5">
              {recommendations.map((rec, i) => (
                <motion.button
                  ...
                  className="w-full flex items-center gap-3 p-4 rounded-2xl bg-card/50 hover:bg-card/80 backdrop-blur-sm border border-border/20 hover:border-primary/20 transition-all text-start group"
          ...
          >
            <div className="card-kid relative overflow-hidden">
              ...
              <div className="relative flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-muted-foreground font-display font-semibold mb-1">{t("home.currentLevel")}</p>
        ...
        <motion.div variants={floatVariants} className="mb-10">
          <h3 className="font-display text-xl sm:text-2xl font-bold mb-5 text-center section-title mx-auto">
            {t("home.freePlay")}
          </h3>
          <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-7 gap-3 sm:gap-4">
            {quickAccessCards.map((card, i) => (
              <motion.button
                ...
                className="game-card-3d group relative overflow-hidden rounded-3xl p-4 sm:p-5 text-center"
              ...
              >
                {/* Background glow */}
                <div
                  className="absolute inset-0 opacity-0 group-hover:opacity-[0.2] dark:group-hover:opacity-[0.25] transition-opacity duration-500 rounded-3xl"
        ...
        <motion.div variants={floatVariants} className="text-center mb-10 flex gap-3 justify-center flex-wrap">
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={() => { playClickSound(); navigate("/levels"); }}
            className="btn-kid gradient-primary text-primary-foreground text-base sm:text-lg px-10 sm:px-12 py-4 shadow-xl"
          >
          ...
          <motion.button
            ...
            className="btn-kid bg-card/80 backdrop-blur-sm text-foreground text-base sm:text-lg px-8 sm:px-10 py-4 border border-border/40 shadow-lg"
          >
          ...
          <Card3D intensity={8}>
            <div className="card-kid text-center relative overflow-hidden">
              ...
              <motion.div
                ...
                animate={{ rotate: [0, 8, -8, 0], y: [0, -5, 0], scale: [1, 1.1, 1] }}
                transition={{ duration: 4, repeat: Infinity }}
              >
                💡
              </motion.div>
