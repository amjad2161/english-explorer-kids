...
const MemoryGame = () => {
  ...
  const startGame = useCallback((catIndex: number) => {
    ...
    setCards(gameCards); setFlipped([]); setMatched([]); setMoves(0); setLastMatchWord(null);
  }, [adaptive, rewards]);
  ...
  }, [flipped, matched, cards, isChecking, moves, stageId, rewards]);
  ...
  return (
    <GameSceneShell
      ...
      progress={progress}
      currentRound={matched.length / 2}
      totalRounds={cards.length / 2 || undefined}
    >
          ...
          {selectedCategory === null ? (
              ...
              {wordCategories.map((cat, index) => (
                ...
                >
                  ...
                  <div className={`bg-gradient-to-br ${categoryGradients[cat.color] || "from-primary to-sunshine"} w-16 h-16 sm:w-20 sm:h-20 rounded-2xl mx-auto mb-3 flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all duration-300`}>
                    <motion.span className="text-3xl sm:text-4xl" whileHover={{ scale: 1.15, rotate: [0, -5, 5, 0] }} transition={{ type: "spring", stiffness: 300 }}>
                      {cat.emoji}
                    </motion.span>
                  </div>
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.3, type: "spring" }} className="mb-5">
                  <StarRating earned={rewards.starsEarned} total={5} size={40} />
                </motion.div>
                ...
                <div className="flex gap-3 justify-center flex-wrap">
                  ...
                  <motion.button whileHover={{ scale: 1.04, y: -2 }} whileTap={{ scale: 0.97 }}
                    onClick={() => { setSelectedCategory(null); rewards.reset(); }}
            ...
            <motion.div key="game" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <div className="card-glass rounded-2xl p-3 mb-5 flex items-center justify-between">
                <motion.button onClick={() => { setSelectedCategory(null); rewards.reset(); }}
                  ...
                  whileHover={{ x: 3 }}>
                  <ArrowRight className="w-3.5 h-3.5 rotate-180" />{t("memory.back")}
                </motion.button>
              ...
              <div className="grid grid-cols-3 md:grid-cols-4 gap-2.5 sm:gap-3 max-w-2xl mx-auto">
                {cards.map((card, i) => {
                  ...
                  return (
                    <motion.div key={card.id} className="aspect-square perspective-1000"
                      ...
                      >
                        ...
                        <div className={`absolute inset-0 rounded-2xl flex flex-col items-center justify-center font-display font-bold text-lg backface-hidden border-2 transition-all duration-300 ${
                          isMatched ? "border-accent/40" : "bg-card border-border/60"
                        }`}
                          style={isMatched ? { background: "hsl(var(--accent) / 0.1)", boxShadow: "0 0 24px hsl(var(--accent) / 0.12), var(--shadow-card)" } : { boxShadow: "var(--shadow-card)" }}
                        >
                          {card.type === "emoji" ? (
