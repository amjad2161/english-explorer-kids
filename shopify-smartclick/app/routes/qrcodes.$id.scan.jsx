...
function assertMyShopifyDomain(shop) {
...
export const loader = async ({ request, params }) => {
  ...
  invariant(shopParam, "Missing shop parameter");

  const shop = assertMyShopifyDomain(shopParam);
  ...
  if (errors?.length) {
    throw new Response(errors[0].message, { status: 502 });
  }
  ...
  try {
    ...
    throw new Response("QR code destination unavailable", { status: 404 });
  }
};
